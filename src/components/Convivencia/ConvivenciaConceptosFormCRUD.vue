    ...
    <v-toolbar flat color="#f0f2f5">
      <v-row>
        <v-col md="auto" style="padding-top:15px; padding-left:20px">
          <span class="headline_dlg" style="color:black">{{ accion }} Convivencia Conceptos</span>
        </v-col>
    ...
    <v-card-text style="padding-left:15px; padding-right:15px">
      <v-container style="padding-left:0px; padding-right:0px">
        ...
        <v-row>
          ...
          <v-col cols="8">
            <v-text-field id="convivencia" v-model.trim="convivencia.convivencia"
...
export default {
  ...
  computed: {
    bloquearSubmit() {
      return (this.convivencia.convivencia.trim() === "" || !this.convivencia.tipoID)
        ? true
        : false;
    },
    institucion() {
  ...
  watch: {
    'random_CC': function () {
      if (this.random_CC) {
        // console.log(this.random_CC);
        this.tipoID = this.convivencia.tipoID;
      }
    },
  },
  ...
  methods: {
    ...
    async ListaConvivenciaTipos() {
      ...
      try {
        ...
        me.convivenciaTipos = response.data;
      } catch (error) {
        console.log(error);
      } finally {
